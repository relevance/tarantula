...
describe Relevance::Tarantula::Crawler do
  ...
  describe "crawling" do
    ...
    # This doesn't really make sense anymore; we're restricting to one initial link per crawl.
    disabled "resets to the initial links/forms on subsequent crawls when times_to_crawl > 1" do
      crawler = Relevance::Tarantula::Crawler.new
  ...
  describe "blip" do
    ...
    describe "messages" do
      ...
      it "blips nothing if verbose (because more elaborate verbose messages serve the same purpose)" do
        $stdout.stubs(:tty?).returns(true)
        crawler = Relevance::Tarantula::Crawler.new
