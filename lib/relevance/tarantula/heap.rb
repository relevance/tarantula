# This is a copy of the Heap class from Kanwei Li's algorithms package:
...
=begin rdoc
    ...
    The Relevance::Tarantula::Heap class is flexible and upon initialization, takes an optional block
    that determines how the items are ordered. Two versions that are included are the
    Relevance::Tarantula::MaxHeap and Relevance::Tarantula::MinHeap that return the largest and smallest items on
    each invocation, respectively.
    
    This library implements a Fibonacci heap, which allows O(1) complexity for most methods.
=end
class Relevance::Tarantula::Heap
  include Enumerable
  ...
  #     heap.pop #=> 1
  def merge!(otherheap)
    raise ArgumentError, "Trying to merge a heap with something not a heap" unless otherheap.kind_of? Relevance::Tarantula::Heap
...
class Relevance::Tarantula::MaxHeap < Relevance::Tarantula::Heap
...
class Relevance::Tarantula::MinHeap < Relevance::Tarantula::Heap
